...
serve(async (req) => {
  ...
  try {
    ...
    const { title, message, target_audience, url = "/" } = body;
    ...
    if (!VAPID_PUBLIC_KEY || !VAPID_PRIVATE_KEY) {
      return new Response(JSON.stringify({ error: "VAPID keys não configuradas" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (userIds.length === 0 && target_audience !== "all") {
      return new Response(JSON.stringify({ sent: 0, message: "Nenhum utilizador encontrado" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    let pushQuery = supabaseAdmin.from("push_subscriptions").select("endpoint, p256dh, auth, user_id");
    if (target_audience !== "all") {
      pushQuery = pushQuery.in("user_id", userIds);
    }

    const { data: pushSubs } = await pushQuery;
    ...
    for (const s of pushSubs) {
      ...
      try {
        ...
        failed++;
        console.log("Push failed for endpoint:", s.endpoint.substring(0, 40), "Error:", e?.message);
      }
    }
